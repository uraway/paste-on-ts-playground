...
export interface EditorProps {
  options: IOptions;
  onChangeOption: (key: keyof IOptions, value: any) => void;
...
export const TSEditor = ({
  ...
  const [inputValue, setInputValue] = useState(options.tsCode);
  ...
  /**
   * Update inputValue when tsCode changes
   */
  useEffect(() => {
    if (options.tsCode !== inputValue) {
      setInputValue(options.tsCode);
    }
  }, [options.tsCode]);

  const onChange = (newValue: string) => {
    setInputValue(newValue);
    onChangeOption('tsCode', newValue);
  };
  ...
  const getDTSForCode = async (model: monaco.editor.ITextModel) => {
    const result = await getEmitResult(model);
    return (
      result?.outputFiles.find((o: any) => o.name.endsWith('.d.ts'))?.text ?? ''
    );
  };
